...
if __name__ == '__main__':
    ...
    for ophys_session_id in experiments.ophys_session_id.unique():
        try:
            print(ophys_session_id, '-', np.where(experiments.ophys_session_id.unique()==ophys_session_id),
    ...
    stim_df.to_hdf(os.path.join(save_dir, 'stimulus_presentations_'+project_code+'_session_'+session_number+'.h5'), key='df')
    print('done')