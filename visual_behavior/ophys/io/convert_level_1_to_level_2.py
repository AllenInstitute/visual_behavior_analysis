...
def save_dataframe_as_h5(df, name, analysis_dir):
    df.to_hdf(os.path.join(analysis_dir, name + '.h5'),
              key='df', format='fixed')
...
def get_segmentation_dir(lims_data):
    ...
    segmentation_folder = sorted(
        [file for file in os.listdir(processed_dir) if 'segmentation' in file])
...
def get_metadata(lims_data, timestamps, core_data):
    ...
    if lims_data.parent_session_id.values[0]:
        metadata['experiment_container_id'] = int(
            lims_data.parent_session_id.values[0])
    else:
    ...
    if len(lims_data['specimen_driver_line'].values[0].split(';')) > 1:
        ...
        metadata['reporter_line'] = lims_data['specimen_reporter_line'].values[0].split('(')[
            0]
    metadata['full_genotype'] = metadata['cre_line'] + \
        ';' + metadata['reporter_line']
    metadata['session_type'] = 'behavior_session_' + \
        lims_data.session_type.values[0].split('_')[-1]
...
def get_stimulus_pkl_path(lims_data):
    ...
    pkl_file = [file for file in os.listdir(
        ophys_session_dir) if '.pkl' in file]
    # remove tiny pkl files (less than 8KB)
    all_files = [os.path.join(ophys_session_dir, pkl_file[i])
                 for i in range(len(pkl_file))]
    all_files_valid = np.array(
        all_files)[[os.path.getsize(i) > 8000 for i in all_files]]
    pkl_file = [os.path.basename(all_files_valid[i])
                for i in range(len(all_files_valid))]
    if len(pkl_file) > 0:
        ...
        if os.name == 'nt':
            ...
            pkl_dir = '\\' + pkl_dir
        pkl_file = [file for file in os.listdir(
            pkl_dir) if file.startswith(expt_date)][0]
...
def get_task_parameters(core_data):
    ...
    task_parameters['response_window'] = [
        core_data['metadata']['response_window']]
    ...
    task_parameters = pd.DataFrame(
        task_parameters, columns=task_parameters.keys(), index=['params'])
...
def save_core_data_components(core_data, lims_data, timestamps_stimulus):
    ...
    running_speed['running_speed'] = medfilt(
        running_speed.running_speed.values, kernel_size=5)
    ...
    # ignore last 10 flashes
    stimulus_table = core_data['visual_stimuli'][:-10]
    if 'omitted_stimuli' in core_data.keys():
        # sometimes there is a key but empty values
        if len(core_data['omitted_stimuli']) > 0:
            ...
            flashes = stimulus_table.merge(
                omitted_flash, how='outer', on='frame')
            flashes['omitted'] = [
                True if omitted is True else False for omitted in flashes.omitted.values]
            flashes = flashes.sort_values(by='frame').reset_index().drop(
                columns=['index']).fillna(method='ffill')
            flashes = flashes[['frame', 'end_frame', 'time',
                               'image_category', 'image_name', 'omitted']]
...
def save_visual_stimulus_data(stimulus_template, stimulus_metadata, lims_data):
    save_dataframe_as_h5(
        stimulus_metadata, 'stimulus_metadata', get_analysis_dir(lims_data))
    save_data_as_h5(stimulus_template, 'stimulus_template',
                    get_analysis_dir(lims_data))
...
def get_roi_locations(lims_data):
    ...
    for i in range(len(rois)):
        ...
        roi_locations_list.append(
            [roi["id"], roi["x"], roi["y"], roi["width"], roi["height"], roi["valid"], mask])
    roi_locations = pd.DataFrame(data=roi_locations_list, columns=[
                                 'id', 'x', 'y', 'width', 'height', 'valid', 'mask'])
...
def add_roi_ids_to_roi_metrics(roi_metrics, roi_locations):
    ...
    for row in roi_metrics.index:
        ...
        id = roi_locations[(roi_locations.x == minx) & (
            roi_locations.y == miny)].id.values[0]
        ids.append(id)
...
def get_roi_metrics(lims_data):
    ...
    roi_locations['unfiltered_cell_index'] = [
        np.where(roi_names == id)[0][0] for id in roi_locations.id.values]
...
def save_roi_metrics(roi_metrics, lims_data):
    save_dataframe_as_h5(roi_metrics, 'roi_metrics',
                         get_analysis_dir(lims_data))


def save_unfiltered_roi_metrics(unfiltered_roi_metrics, lims_data):
    save_dataframe_as_h5(unfiltered_roi_metrics,
                         'unfiltered_roi_metrics', get_analysis_dir(lims_data))
...
def sql_lims_query(query):
    ...
    conn = psycopg2.connect(dbname="lims2", user="limsreader",
                            host="limsdb2", password="limsro", port=5432)
    cur = conn.cursor()
...
def add_cell_specimen_ids_to_roi_metrics(lims_data, roi_metrics, cache_dir):
    ...
    if len(ids) > 0:
        try:
            df = pd.DataFrame(ids, columns=[
                              'mouse', 'container_id', 'cell_specimen_id', 'valid_roi', 'cell_roi_id'])
            ...
            print(
                'something bad happened when trying to get cell specimen ids from lims, setting to None')
            print(e)
            roi_metrics['cell_specimen_id'] = None
    else:
...
def save_roi_masks(roi_masks, lims_data):
    f = h5py.File(os.path.join(
        get_analysis_dir(lims_data), 'roi_masks.h5'), 'w')
...
def save_corrected_fluorescence_traces(corrected_fluorescence_traces, roi_metrics, lims_data):
    traces_path = os.path.join(get_analysis_dir(
        lims_data), 'corrected_fluorescence_traces.h5')
...
def get_dff_traces(roi_metrics, lims_data):
    dff_path = os.path.join(get_ophys_experiment_dir(
        lims_data), str(get_lims_id(lims_data)) + '_dff.h5')
    ...
    print(dff_traces.shape)
    roi_metrics = roi_metrics[roi_metrics.cell_index.isin(
        bad_cell_indices) == False]
...
def save_timestamps(timestamps, dff_traces, core_data, roi_metrics, lims_data):
    ...
    if dff_traces.shape[1] < timestamps['ophys_frames']['timestamps'].shape[0]:
        difference = timestamps['ophys_frames']['timestamps'].shape[0] - \
            dff_traces.shape[1]
    ...
    if dff_traces.shape[1] > timestamps['ophys_frames']['timestamps'].shape[0]:
        difference = timestamps['ophys_frames']['timestamps'].shape[0] - \
            dff_traces.shape[1]
        logger.info(
            'length of ophys timestamps <  length of traces by %s frames, truncating traces', str(difference))
        dff_traces = dff_traces[:,
                                :timestamps['ophys_frames']['timestamps'].shape[0]]
    ...
    if len(running_speed) < timestamps['stimulus_frames']['timestamps'].shape[0]:
        timestamps['stimulus_frames']['timestamps'] = timestamps['stimulus_frames']['timestamps'][:len(
            running_speed)]
    save_dataframe_as_h5(timestamps, 'timestamps', get_analysis_dir(lims_data))


def get_motion_correction(lims_data):
    csv_file = [file for file in os.listdir(
        get_processed_dir(lims_data)) if file.endswith('.csv')]
...
def get_average_image(lims_data):
    average_image = mpimg.imread(os.path.join(
        get_segmentation_dir(lims_data), 'avgInt_a1X.png'))
...
def run_roi_validation(lims_data, cache_dir):
    ...
    roi_metrics = add_cell_specimen_ids_to_roi_metrics(
        lims_data, roi_metrics, cache_dir)
...
def convert_level_1_to_level_2(lims_id, cache_dir=None, plot_roi_validation=True):
    ...
    analysis_dir = get_analysis_dir(
        lims_data, cache_on_lims_data=True, cache_dir=cache_dir)
    ...
    corrected_fluorescence_traces = get_corrected_fluorescence_traces(
        roi_metrics, lims_data)
    save_corrected_fluorescence_traces(
        corrected_fluorescence_traces, roi_metrics, lims_data)
