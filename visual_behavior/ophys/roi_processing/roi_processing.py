...
def exp_roi_metrics_dataframe(experiment_id, mask_shift=False, include_failed_rois=True):
    ...
    if include_failed_rois == False:
        roi_metrics_df = roi_metrics_df.loc[roi_metrics_df["valid_roi"] == True]
...
def change_mask_from_bool_to_binary(mask):
    """change a mask from bool to binary so the backgroun is 0s
       and transparent, which allows it to be plotted over another image
...
def gen_blank_mask_of_FOV_dimensions(experiment_id):
    """generates a transparent mask the same dimensions as the microscope
       FOV after motion correction & cell segmentation
