...
def clean_roi_locations_column_labels(roi_locations_dataframe):
    """takes some column labels from the roi_locations dataframe and
       renames them to be more explicit and descriptive, and to match the
       column labels from the objectlist dataframe.
...
def get_failed_roi_exclusion_labels(ophys_experiment_id):
    """Queries LIMS  roi_exclusion_labels table via AllenSDK PostgresQuery
       ...
       Failed rois will be listed multiple times/in multiple rows depending
       upon how many exclusion labels they have.
