...
def filter_dff_traces_crosstalk(sessions):
    for ses in sessions:
        ...
        for pair in pairs:
            ...
            ica_obj.dff_files = {}
            ica_obj.dffs = {}
            ica_obj.dff_ct_files = {}
            ica_obj.dffs_ct = {}
            for pkey in ica_obj.pkeys:
                tkey='roi'
                ica_obj.dff_files[pkey] = os.path.join(ica_obj.session_dir, f"{ica_obj.exp_ids[pkey]}_dff.h5")
                with h5py.File(ica_obj.dff_files[pkey], 'r') as f:
                    ica_obj.dffs[pkey] = f['data']
                traces_dict = {}
                assert ica_obj.dffs[pkey].shape[0] == len(ica_obj.rois_names_valid[pkey][tkey]), f"Dff traces are not aligned to validation json for {ica_obj.exp_ids[pkey]}"
                for i in range(len(ica_obj.rois_names_valid[pkey][tkey])):
                    roi_name = ica_obj.rois_names_valid[pkey][tkey][i]
                    traces_dict[roi_name] = ica_obj.dffs[pkey][i]
                traces_dff_ct = [traces for roi_name, traces in traces_dict.items() if
                                 ica_obj.rois_valid_ct[pkey][roi_name]]
                dffs_ct_path = add_suffix_to_path(ica_obj.dff_files[pkey], '_ct')
                if not os.path.isfile(dffs_ct_path):
                    with h5py.File(dffs_ct_path, "w") as f:
                        ...
                        f.create_dataset("roi_names", data=ica_obj.rois_valid_ct[pkey])
                else:
                    with h5py.File(dffs_ct_path, "r") as f:
                        traces_dff_ct = f["data"][()]
                ica_obj.dffs_ct[pkey][tkey] = traces_dff_ct
                ica_obj.dff_ct_files[pkey] = dffs_ct_path
    return
