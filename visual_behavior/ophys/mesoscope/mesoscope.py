...
class MesoscopeDataset(object):
    ...
    def stitch_full_field(self, summ=True):
        ...
        full_field_tiff_path = self.get_full_field_tiff()
        image_stitched = np.zeros([slices, pixelResY, pixelResX * roi_num], dtype='int16')
        if full_field_tiff_path != '':
            ...
            roi_num = len(rois)
            pixelResY = rois[1]['scanfields']['pixelResolutionXY'][1]
            pixelResX = rois[1]['scanfields']['pixelResolutionXY'][0]
            image_nlines = image.shape[1]
            image_npixels = image.shape[2]
            y_gap = np.int16((image_nlines - pixelResY * roi_num) / (roi_num - 1))
            image_stitched = np.zeros([slices, pixelResY, pixelResX * roi_num], dtype='int16')
            ...
            for j in range(slices):
                for i in range(roi_num):
                    image_stitched[j, :, i * image_npixels:(i + 1) * image_npixels] = image[j, i * (pixelResY + y_gap):(i + 1) * pixelResY + i * y_gap, :]
            if summ:
            ...
            full_field_path = ''
            logger.error("Full field tiff is not in session folder, provide offline path")
        return image_stitched, meta
