...
def get_cell_matching_dataset_dict(lims_ids, cache_dir):
    ...
    for lims_id in lims_ids:
        cell_matching_dataset_dict[lims_id] = CellMatchingDataset(lims_id, cache_dir = cache_dir, from_processed_data=False)
...
def get_cell_matching_results_cell_specimen_ids(container_id):
    ...
    matching_df.to_csv(os.path.join(get_container_analysis_dir(container_id),str(int(container_id))+'_matching_results_cell_specimen_ids.csv'))
    return matching_df
...
def get_valid_cell_matching_results_cell_specimen_ids(container_id, cell_matching_dataset_dict):
    ...
    valid_matching_df.to_csv(os.path.join(get_container_analysis_dir(container_id),str(int(container_id))+'_matching_results_valid_cell_specimen_ids.csv'))
...
def plot_max_proj_around_mask_coordinates(dataset,mask,unique_cell_id,cell_specimen_id,background_image,space,ax=None):
    if ax is None:
        fig,ax = plt.subplots()
    xmin,xmax,ymin,ymax = get_coordinates(mask)
    ax.imshow(background_image, cmap='gray', vmin=0, vmax=np.amax(background_image))
