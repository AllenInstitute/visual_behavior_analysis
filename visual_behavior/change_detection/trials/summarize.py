...
DEFAULT_SESSION_METRICS = dict(
    session_id=session_metrics.session_id,
    session_duration=session_metrics.session_duration,
    d_prime_peak=session_metrics.peak_dprime,
    d_prime=lambda session_trials: session_metrics.discrim(session_trials, 'change', 'detect', metric=metrics.d_prime),
    discrim_p=lambda session_trials: session_metrics.discrim(session_trials, 'change', 'detect', metric=metrics.discrim_p),
    response_bias=lambda session_trials: session_metrics.response_bias(session_trials, 'detect'),
    ...
    hit_rate=lambda session_trials: session_metrics.discrim(session_trials, 'change', 'detect', metric=metrics.hit_rate),
    false_alarm_rate=lambda session_trials: session_metrics.discrim(session_trials, 'change', 'detect', metric=metrics.false_alarm_rate),
...
summarize_session = create_summarizer(DEFAULT_SESSION_METRICS)

def session_level_summary(trials, **kwargs):
    """ computes session-level summary table
    """

    metrics = DEFAULT_SESSION_METRICS.copy()
    metrics.update(kwargs)

    summarizer = create_summarizer(**metrics)
