...
# here we use flash-aligned traces to compute flash evoked response.
# in omissions_traces_peaks_quantify, we used .75sec before omission-aligned traces to quantify flash evoked responses. The problem is that not all flashes happen every .75sec (due to problems like frame drops, apparently.)

from def_funs import *

def omissions_traces_peaks_quantify_flashAl(traces_aveTrs_time_ns, traces_aveTrs_time_ns_f, bl_preOmit, bl_preFlash, mean_notPeak, cre, peak_win, flash_win, samps_bef, frame_dur, doShift_again, doPeakPerTrial, doROC, doPlots=0, index=0, local_fluo_allOmitt=0, num_omissions=0):
    ...
    list_times, list_times_flash, list_times_flash_timing = set_frame_window_flash_omit(peak_win, flash_win, flash_win, samps_bef, frame_dur)
...
#     this_sess.at[index, ['peak_timing_eachN_traceMed']] = [peak_timing_eachN_traceMed] # neurons
    ...
    # for each trial-median neuron trace find the peak during the peak_win window
    if mean_notPeak: # compute average trace during peak_win
        peak_amp_eachN_traceMed_flash = np.nanmean(traces_aveTrs_time_ns_f[list_times_flash], axis=0) # neurons
    else: # compute peak (unlike omissions, we know all cell lines respond to flash, so we just get the max)
        peak_amp_eachN_traceMed_flash = np.nanmax(traces_aveTrs_time_ns_f[list_times_flash], axis=0) # neurons
    ...
    peak_timing_eachN_traceMed_flash = np.nanargmax(traces_aveTrs_time_ns_f[list_times_flash_timing], axis=0) # neurons
    ...
    peak_timing_eachN_traceMed_flash = peak_timing_eachN_traceMed_flash + (list_times_flash_timing[0] - samps_bef)    
    # old: when we computed flash timing from omission aligned traces
...
#     this_sess.at[index, ['peak_timing_eachN_traceMed_flash']] = [peak_timing_eachN_traceMed_flash] # neurons
    ...
    # you need to also add it for local_fluo_flashBefOmitt... for now not doing it bc doPeakPerTrial is 0 by default.
    if doPeakPerTrial:
