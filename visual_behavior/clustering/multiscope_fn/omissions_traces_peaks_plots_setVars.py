...
#%%
useSDK = 1 # load the all_sess file created by using allenSDK in script load_behavior_ophys_dataset_fn.py.
doCorrs = -1 # if 0, compute omit-aligned trace median, peaks, etc. If 1, compute corr coeff between neuron pairs in each layer of v1 and lm. If -1, only get the traces, dont compute peaks, mean, etc.  
