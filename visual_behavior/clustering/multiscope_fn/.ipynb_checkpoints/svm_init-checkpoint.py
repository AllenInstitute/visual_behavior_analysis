...
To perform SVM (do this on the cluster), it will call "svm_main" (for this, you will need to run svm_init_pbs on the cluster which calls svm_main_pbs).
...
flash_win = [-.75, 0] # previous value: # [-.75, -.25]
flash_win_vip = [-1, -.25] # previous value (for flash_win of all cre lines): # [-.75, -.25] # 
...
#    svm_vars = pd.read_hdf(svmName, key='svm_vars')
    ...
    this_sess = svm_main_post(session_id, experiment_ids, validity_log_all, dir_svm, frames_svm, all_sess, same_num_neuron_all_planes, use_ct_traces, mean_notPeak, peak_win, flash_win, flash_win_vip, flash_win_timing, bl_percentile, cols, doShift_again, analysis_dates, doPlots=0)
