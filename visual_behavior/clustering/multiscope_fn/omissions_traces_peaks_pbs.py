...
#%%
def omissions_traces_peaks_pbs(session_id, experiment_ids, validity_log_all, norm_to_max, mean_notPeak, peak_win, flash_win, flash_win_timing, flash_win_vip, bl_percentile, num_shfl_corr, trace_median, doScale, doShift, doShift_again, bl_gray_screen, samps_bef, samps_aft, doCorrs, subtractSigCorrs, saveResults, cols, use_ct_traces=1, doPlots=0, doROC=0):    
...
#         return this_sess
    ...
    #                 sum(num_trials_perImageType) == local_fluo_allOmitt.shape[2]
                ...
                this_sess_l.at[index, 'list_flashes'] = list_flashes # flash onset
                this_sess_l.at[index, 'list_omitted'] = list_omitted # omission onset
                this_sess_l.at[index, 'running_speed'] = behav_data['running_speed']
                this_sess_l.at[index, 'licks'] = behav_data['licks'] 
                this_sess_l.at[index, 'rewards'] = behav_data['rewards']            
                #'running_speed', 'licks', 'rewards', 'd_prime', 'hit_rate', 'catch_rate'


    #            local_fluo_allOmitt_allPlanes.append(local_fluo_allOmitt)
                if doCorrs==-1:
                    this_sess = this_sess.iloc[:,range(13)].join(this_sess_l) # len(cols_basic) = 13
    ...
    #                 (traces_aveTrs_time_ns, bl_preOmit, bl_preFlash, mean_notPeak, cre, peak_win, flash_win, flash_win_timing, flash_index, bl_index_pre_omit, bl_index_pre_flash, bl_percentile, samps_bef, frame_dur, doShift_again, doPeakPerTrial, doROC, local_fluo_allOmitt, index, mouse, flashes_win_trace_index_all, num_omissions, doPlots)
                    '''
...
num_shfl_corr = 5 #50 # set to 0 if you dont want to compute corrs for shuffled data # shuffle trials, then compute corrcoeff... this serves as control to evaluate the values of corrcoeff of actual data    
