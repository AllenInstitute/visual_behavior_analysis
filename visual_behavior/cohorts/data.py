...
def annotate_mouse_info(trials):
    ...
    mouse_df = pd.Series(trials['mouse_id'].unique()) \
...
def get_training_day(df_in):
    ...
    day_zero = {
        r['mouse']: r['day_zero'].strftime("%Y-%m-%d")
        for _, r in cohort_assignment.iterrows()
    }
    coh = cohort_assignment.set_index('mouse')['cohort']  # NOQA: F841
    ...
    for mouse, group in df_in.groupby('mouse_id'):
        ...
        try:
            ...
            offset = 0
        training_day_lookup[mouse] = {
            date: training_day - offset
            for training_day, date in enumerate(dates)
        }

    return df_in.apply(
        lambda row: training_day_lookup[row['mouse_id']][row['date']],
        axis=1
    )
