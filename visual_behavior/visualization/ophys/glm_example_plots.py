...
def plot_glm_features_for_window(dataset, xlim_seconds, title=None,
    ...
    # standalone = True when this function manages the figure; False when
    # an outer composite supplied the axes array.  Side effects below
    # (plt.suptitle, plt.subplots_adjust, save_figure) only run in standalone mode.
...
def plot_weights_and_coding_score_heatmaps_for_experience_levels_main_figure(kernel, weights_df, run_params,
    ...
    if row_condition == 'cluster_id':
        ...
        figsize = [6, 4]
    # standalone = True when this function manages the figure; False when an
    # outer composite supplied `fig` (and an optional `bbox` sub-rectangle).
    ...
    if standalone and add_suptitle:
