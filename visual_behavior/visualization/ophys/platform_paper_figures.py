...
def plot_population_averages_for_conditions(multi_session_df, data_type, event_type, axes_column, hue_column,
    ...
    if legend:
        if hue_column == 'passive':
            ...
            ax[i].legend(title=hue_column, loc='upper center', fontsize='x-small', title_fontsize='x-small',
                         bbox_to_anchor=(1.2, 1))
    if project_code:
        if suptitle is None:
            suptitle = 'population average - ' + data_type + ' response - ' + project_code[14:]
...
def plot_mean_response_by_epoch(df, metric='mean_response', horizontal=True, ymin=0, ymax=None, ylabel='mean response', estimator=np.mean,
...
def plot_mean_response_by_epoch_all_cell_types(df, metric='mean_response', horizontal=True, ymin=0, ymax=None,
    ...
    if horizontal:
        ax.set_xlabel('Epoch within session (5 min bins)')
    else:
        ax.set_xlabel('')
    ax.set_xlabel('Epoch within session (5 min bins)')
    ax.tick_params(axis='both', which='major', labelsize=14)
    ...
    if x_axis_col: 
        ...
        if horiz:
            ax[0].annotate(annot[0], xy=(lims[0]-0.1, -0.05), xycoords=ax[0].get_xaxis_transform(), ha="right", va="center",
                        fontsize=10)
            ax[2].annotate(annot[1], xy=(lims[1]+0.1, -0.05), xycoords=ax[2].get_xaxis_transform(), ha="left", va="center",
                        fontsize=10)
