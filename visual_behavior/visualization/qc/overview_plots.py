...
def plot_expts_for_container(experiments_table, container_id, experiment_ids_to_highlight=None, max_n_expts=None,
                             ax=None):
    expts = experiments_table[experiments_table.container_id == container_id]
    ...
    super_container_id = expts.super_container_id.values[0]
    ax.text(x=-2.5, y=0, s=str(super_container_id) + '_' + str(container_id), ha='center', va='bottom', fontsize=14)
...
def plot_container_overview(experiments_table, project_code):
    ...
    container_ids = np.sort(project_expts.container_id.unique())
    max_n_expts = project_expts.groupby('container_id').count().ophys_experiment_id.max()

    figsize = (15, len(container_ids))
    fig, ax = plt.subplots(len(container_ids), 1, figsize=figsize, gridspec_kw={'wspace': 0, 'hspace': 0})
