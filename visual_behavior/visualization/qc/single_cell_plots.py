...
#                 window = rp.get_default_omission_response_params()["window_around_timepoint_seconds"]
#                 cell_data = odf[(odf.cell_specimen_id==cell_specimen_id)]
#                 ax[i+(n*2)] = sf.plot_mean_trace_from_mean_df(cell_data,
#                                                     frame_rate=analysis.ophys_frame_rate, ylabel=ylabel,
#                                                     legend_label=image_name, color='gray', interval_sec=1,
#                                                     xlims=window, ax=ax[i+(n*2)])
#                 ax[i+(n*2)] = sf.plot_flashes_on_trace(ax[i+(n*2)], analysis, window=window, trial_type=None, omitted=True, alpha=0.15, facecolor='gray')
#                 ax[i+(n*2)].set_title(container_expts.loc[ophys_experiment_id].session_type[6:]+'\n omission response')
#
#
#                 window = rp.get_default_trial_response_params()["window_around_timepoint_seconds"]
...
#                 for c, hit in enumerate([True, False]):
#                     if len(cell_data[cell_data.hit==hit])>0:
#                         ax[i+(n*3)] = sf.plot_mean_trace_from_mean_df(cell_data[cell_data.hit==hit],
#                                                             frame_rate=analysis.ophys_frame_rate, ylabel=ylabel,
#                                                             legend_label=hit, color=hit_colors[c], interval_sec=1,
#                                                             xlims=window, ax=ax[i+(n*3)])
#                 ax[i+(n*3)] = sf.plot_flashes_on_trace(ax[i+(n*3)], analysis, window=window, trial_type='go', omitted=False, alpha=0.15, facecolor='gray')
#                 ax[i+(n*3)].set_title(container_expts.loc[ophys_experiment_id].session_type[6:]+'\n change response')
#         except Exception as e:
#             print(ophys_experiment_id)
#             print(e)
#     fig.tight_layout()
#     fig.suptitle(str(cell_specimen_id)+'_'+dataset.metadata_string, x=0.5, y=1.01, horizontalalignment='center')
#     if save_figure:
#         save_dir = utils.get_single_cell_plots_dir()
#         utils.save_figure(fig, figsize, save_dir, 'across_session_responses', str(cell_specimen_id)+'_'+dataset.metadata_string+'_across_session_responses_'+suffix)
#         plt.close()
