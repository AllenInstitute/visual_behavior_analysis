from __future import print_function  # NOQA: F401
import pandas as pd
...
def make_IC_plots(path, events, traces, IC_masks, figsize=(9, 6, )):
    ...
    for ii, IC_num in enumerate(np.sort(IC_masks.keys())):
        ...
        #     break
        fig.clf()
        ax_IC_mask = vbp.placeAxesOnGrid(
            fig,
            xspan=(0, 0.45, ),
            yspan=(0, 0.5, )
        )
        ax_IC_ts = vbp.placeAxesOnGrid(
            fig,
            xspan=(0.575, 1, ),
            yspan=(0, 0.40, )
        )
        ax_ev_trig_avg = []
        ax_ev_trig_avg.append(
            vbp.placeAxesOnGrid(fig, xspan=(0, 0.3), yspan=(0.57, 1))
        )
        ax_ev_trig_avg.append(
            vbp.placeAxesOnGrid(fig, xspan=(0.356, 0.656), yspan=(0.57, 1))
        )
        ax_ev_trig_avg.append(
            vbp.placeAxesOnGrid(fig, xspan=(0.7, 1), yspan=(0.57, 1))
        )
        ...
        dat_vis = ut.event_triggered_average(
            trace,
            mask=None,
        ...
        dat_aud = ut.event_triggered_average(
            trace,
            mask=None,
            events=auditory_events,
            frame_before=20,
            frame_after=100,
            sampling_rate=20,
            output='f',
        ...
        dat_vis_aud = ut.event_triggered_average(
            trace,
            mask=None,
            events=visual_auditory_events,
            frame_before=20,
            frame_after=100,
            sampling_rate=20,
            output='f',
        ...
        titles = [
            'visual triggered avg',
            'auditory triggered avg',
            'vis/aud triggered avg',
        ]
        for jj in range(3):
        ...
        vbp.save_figure(
            fig,
            os.path.join(savepath, 'IC{}'.format(IC_num)),
            formats=['.png'],
            figsize=figsize
        )
...
def run(path):
    ...
    event_frames = get_event_frames(
        stim_df,
        behavior_frame_times,
        f_frame_times
    )
