...
#     from sklearn.svm import SVC
    ...
    if options.get('l1'):
        ...
        l2 = options.get('l2');        
#         linear_svm = svm.LinearSVC(C=l2, loss='squared_hinge', penalty='l2', dual=True) # max_iter=2000 # default max_iter=1000 # multi_class='ovr'
        linear_svm = svm.LinearSVC(C=l2, loss='squared_hinge', penalty='l2', dual=False) 

          # linear, poly, rbf, sigmoid
#         linear_svm = svm.SVC(kernel='linear', C=l2) #, decision_function_shape='ovo') # one-vs-one (‘ovo’) is always used as multi-class strategy.
#         linear_svm = svm.SVC(kernel='poly', degree=3, C=l2) 
...
#     print(linear_svm.get_params)
    ...
    perClassErrorTest = (1-linear_svm.score(XTest, YTest))*100
    perClassErrorTrain = (1-linear_svm.score(XTrain, YTrain))*100
    
    # the following two are the same:
#     linear_svm.score(XTest, YTest)
#     np.mean(YTest == linear_svm.predict(XTest))

    #%% below only works for 2 classes
    '''
...
#    from linearSVM import linearSVM
    ...
    # below works only for 2 classes. use this instead:     perClassErrorTest = (1-linear_svm.score(XTest, YTest))*100
    '''
    def perClassError(Y, Yhat):
        perClassEr = np.sum(abs(np.squeeze(Yhat).astype(float)-np.squeeze(Y).astype(float)))/len(Y)*100
        return perClassEr
...
#    nFrs = np.shape(X)[0]
    if num_classes==2:
        wAllC = np.ones((numSamples, nCvals, X.shape[0])) + np.nan # (1, n_features)
        bAllC = np.ones((numSamples, nCvals)) + np.nan # (1,) 
    else:
        wAllC = np.ones((numSamples, nCvals, num_classes, X.shape[0])) + np.nan # (n_classes, n_features)
        bAllC = np.ones((numSamples, nCvals, num_classes)) + np.nan # (n_classes,)
...
#             print(f'\tc = {cvect[i]}')
            ...
            # coef_ only exists for the linear kernel
#             print(summary.model.intercept_.shape)
            wAllC[s,i] = np.squeeze(summary.model.coef_) # weights of all neurons for each value of c and each shuffle # shape (1, n_features) if n_classes == 2 else (n_classes, n_features)
            bAllC[s,i] = np.squeeze(summary.model.intercept_) # shape (1,) if n_classes == 2 else (n_classes,)
            ...
            perClassErrorTest_shfl[s,i] = (1 - summary.model.score(summary.XTest, summary.YTest[permIxs]))*100
            perClassErrorTest_chance[s,i] = (1 - summary.model.score(summary.XTest, Y_chance))*100
            
            # commenting below bc it only works for 2 classes.
