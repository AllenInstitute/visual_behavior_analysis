...
svm_blocks = -100 #-1 #-100 # 2 # -101: run the analysis only on engaged trials # -1: divide trials based on some engagement metric (defined by engagement_pupil_running below) # 2 # number of trial blocks to divide the session to, and run svm on. # -100: run svm analysis on the whole session
engagement_pupil_running = 0 # only applicable when svm_blocks=-1 # np.nan or 0,1,2 for engagement, pupil, running: which metric to use to define engagement? 
...
# before running the code below, you need to run the following in the terminal: pip install simple_slurm
