...
for cre in cres: 
    ...
    thiscre = thiscre.rename(columns={'peak_amp_allPlanes_allExp':'decoding_magnitude'})

    # take testing dataset decoding accuracy
    test = np.array([thiscre['decoding_magnitude'].values[i][1] for i in range(thiscre.shape[0])])
    thiscre['decoding_magnitude'] = list(test)
